...
```{r,echo=T, warning= FALSE}
kable(as.data.frame(table(TED.tr$cate)) %>% rename(Topic = Var1, Count = Freq) , caption = "The number of observations per Topic", caption.above = TRUE) %>%
...
kable(as.data.frame(table(TED.tr.subs$cate)) %>% rename(Topic = Var1, Count = Freq) , caption = "The number of observations per Topic, after sub-sampling", caption.above = TRUE) %>%
   kable_styling(bootstrap_options = "bordered") %>%
   kableExtra::scroll_box(width = "100%", height = "250px")
```
