...
data.frame(doc.freq,
  ...
  geom_smooth(method="lm",
              formula = 'y ~ x') +
  labs(
    title="Correlation between number of tokens and LSA dimension 1 values",
    x="Number of tokens",
    y="LSA dim. 1"
  )
```
...
ggplot(data=TED.lsa.source,mapping = aes(
  ...
  labs(x = "dimension2",
       y = "dimension3",
       title = "Distribution of texts in different category on LSA(TF) dimension 2 and 3")+
...
ggplot(data=TED.lsa.source,mapping = aes(
  ...
  labs(x = "dimension3",
       y = "dimension4",
       title = "Distribution of texts in different category on LSA(TF) dimension 3 and 4")+
  scale_colour_discrete(
    name="Category",
    breaks=c("1","2","3"),
    labels=c("AI","Climate change","Relationships")
